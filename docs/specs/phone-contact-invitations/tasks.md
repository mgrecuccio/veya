...
- [ ] Ensure `POST /api/v1/contacts/invitations` accepts `phoneNumber` in E.164 format.
...
- [ ] Ensure incoming pending invitations expose `senderPhoneNumber` and an optional safe display name.
...
- [x] Use country, national-number, and optional nickname controls on the Contacts page.
- [x] Reuse the shared phone country and normalization utilities.
- [x] Use phone-specific invitation copy, attributes, validation, and placeholders.
...
- [x] Define `SendInvitationRequest.phoneNumber`.
...
- [x] Use `senderPhoneNumber` in the pending invitation model and as the display fallback.
...
- [x] Confirm every invitation request, UI copy, and test fixture uses the phone number as the invitee identifier.
